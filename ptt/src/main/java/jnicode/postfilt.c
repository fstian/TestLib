...
static s16 res2_buf[PIT_MAX+L_SUBFR];
static s16 *res2;
static s16 scal_res2_buf[PIT_MAX+L_SUBFR];
static s16 *scal_res2;

        // memory of filter 1/A(z/GAMMA1_PST)  

static s16 mem_syn_pst[M];*/
...
{
  pG729DecH->res2  = pG729DecH->res2_buf + PIT_MAX;
  pG729DecH->scal_res2  = pG729DecH->scal_res2_buf + PIT_MAX;

  Set_zero(pG729DecH->mem_syn_pst, M);
  Set_zero(pG729DecH->res2_buf, PIT_MAX+L_SUBFR);
  Set_zero(pG729DecH->scal_res2_buf, PIT_MAX+L_SUBFR);
...
)
{
 ...
 s16 Ap3[MP1], Ap4[MP1];  /* bandwidth expanded LP parameters */

 s16 *Az;                 /* pointer to Az_4:                 */
                             /*  LPC parameters in each subframe */
 s16   t0_max, t0_min;    /* closed-loop pitch search range   */
 s16   i_subfr;           /* index for beginning of subframe  */
 ...
 l32  L_tmp;
    ...
    *-----------------------------------------------------*/
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
      ...
      Residu(Ap3, &syn[i_subfr], pG729DecH->res2, L_SUBFR);

      /* scaling of "res2[]" to avoid energy overflow */

      for (j=0; j<L_SUBFR; j++)
      {
        pG729DecH->scal_res2[j] = shr(pG729DecH->res2[j], 2);
      }

      /* pitch postfiltering */
      if (sub(Vad, 1) == 0)
        pit_pst_filt(pG729DecH->res2, pG729DecH->scal_res2, t0_min, t0_max, L_SUBFR, res2_pst);
      else
        for (j=0; j<L_SUBFR; j++)
          res2_pst[j] = pG729DecH->res2[j];
      ...
      preemphasis(res2_pst, temp2, L_SUBFR, pG729DecH);

      /* filtering through  1/A(z/GAMMA1_PST) */

      Syn_filt(Ap4, res2_pst, &syn_pst[i_subfr], L_SUBFR, pG729DecH->mem_syn_pst, 1);

      /* scale output to input */

      agc(&syn[i_subfr], &syn_pst[i_subfr], L_SUBFR, pG729DecH);

      /* update res2[] buffer;  shift by L_SUBFR */

      Copy(&pG729DecH->res2[L_SUBFR-PIT_MAX], &pG729DecH->res2[-PIT_MAX], PIT_MAX);
      Copy(&pG729DecH->scal_res2[L_SUBFR-PIT_MAX], &pG729DecH->scal_res2[-PIT_MAX], PIT_MAX);
...
{
  s16 i, j, t0;
  s16 g0, gain, cmax, en, en0;
  s16 *p, *p1, *deb_sig;
  l32 corr, cor_max, ener, ener0, temp;
  l32 L_temp;
 ...
 *---------------------------------------------------------------------------*/
  ...
  j = norm_l(temp);
  cmax = round_linux(L_shl(cor_max, j));
  en = round_linux(L_shl(ener, j));
  en0 = round_linux(L_shl(ener0, j));
  ...
  if (temp < (l32)0)           /* if prediction gain < 3 dB   */
...
//  static s16 mem_pre = 0;
  ...
  *p1 = sub(*p1, mult(g, pG729DecH->mem_pre));

  pG729DecH->mem_pre = temp;
...
{
//  static s16 past_gain=4096;         /* past_gain = 1.0 (Q12) */
  s16 i, exp;
  s16 gain_in, gain_out, g0, gain;                     /* Q12 */
  l32 s;
  ...
  if (s == 0) {
    pG729DecH->past_gain = 0;
    return;
  }
  exp = sub(norm_l(s), 1);
  gain_out = round_linux(L_shl(s, exp));
