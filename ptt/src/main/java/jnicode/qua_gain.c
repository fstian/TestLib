...
static void Gbk_presel(
   s16 best_gain[],     /* (i) [0] Q9 : unquantized pitch gain     */
                           /* (i) [1] Q2 : unquantized code gain      */
   s16 *cand1,          /* (o)    : index of best 1st stage vector */
   s16 *cand2,          /* (o)    : index of best 2nd stage vector */
   s16 gcode0           /* (i) Q4 : presearch for gain codebook    */
);
...
s16 Qua_gain(
   ...
   s16 tameflag,      /* (i)     : set to 1 if taming is needed  */
   TG729BENCODER *pG729EncH
)
{
   s16  i, j, index1, index2;
   s16  cand1, cand2;
   s16  exp, gcode0, exp_gcode0, gcode0_org, e_min ;
   s16  nume, denom, inv_denom;
   s16  exp1,exp2,exp_nume,exp_denom,exp_inv_denom,sft,tmp;
   s16  g_pitch, g2_pitch, g_code, g2_code, g_pit_cod;
   s16  coeff[5], coeff_lsf[5];
   s16  exp_min[5];
   l32  L_gbk12;
   l32  L_tmp, L_dist_min, L_temp, L_tmp1, L_tmp2, L_acc, L_accb;
   s16  best_gain[2];
...
// static s16 past_qua_en[4] = { -14336, -14336, -14336, -14336 };
   ...
   *---------------------------------------------------*/
   Gain_predict( pG729EncH->past_qua_en, code, L_subfr, &gcode0, &exp_gcode0 );
...
if(tameflag == 1){
   ...
   *----------------------------------------------*/
   Gain_update( pG729EncH->past_qua_en, L_gbk12 );

   return( add( (s16)(map1[index1]*(s16)NCODE2), map2[index2] ) );
...
)
{
   s16    acc_h;
   s16    sft_x,sft_y;
   l32    L_acc,L_preg,L_cfbg,L_tmp,L_tmp_x,L_tmp_y;
   l32 L_temp;
