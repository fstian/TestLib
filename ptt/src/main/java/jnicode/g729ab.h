...
l32  g729_encode(
				 ...
				 s16 inLength,     //input: encoder data length
				 u8 * pOutPut,   //output: encoded data
				 s16 * outLength); //output: length in s8 unit.

l32  g729_decode(							//output:
				 HG729BDEC pG729,			//input: the input decoder handle
                 u8 * pDecbitstream,	//input: the decoder data
				 s16 inlength,				//input: length of the input length
				 s16* pDecOut,				//output: decoded data
				 s16 * poutLength			//output: length in s8 unit.
				 );
