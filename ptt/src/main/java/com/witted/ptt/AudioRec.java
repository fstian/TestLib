            ...
            public void handleMessage(@NonNull Message msg) {
                ...
                try {
                    ...
                    if(read>0){
                        ...
                        Timber.i("mDatagramSocket.send %s    port%s   code%s",mIp,mPort,mCode);
...
//            while (mStart) {
//                int read = mAudioRecord.read(mRecordDatas, 0, mRecordDatas.length);
//                Timber.i("state:%s", read);
//
//                if (read > 0) {
//                    DatagramPacket dp = null;
//                    try {
//                        encodec(mRecordDatas, mEncodeDatas);
//                        dp = new DatagramPacket(mEncodeDatas, 0, mEncodeDatas.length, InetAddress.getByName(mIp), mPort);
//                    } catch (Exception e) {
...
//        short[] aecTmpIn = new short[cacheSize ];
//        short[] aecTmpOut = new short[cacheSize ];
//
//        //320的byte数组转160的short数组
//        ByteBuffer.wrap(src).order(ByteOrder.LITTLE_ENDIAN)
//                .asShortBuffer().get(aecTmpIn);
//
//        aecm.farendBuffer(aecTmpIn, cacheSize );
//
//        aecm.echoCancellation(aecTmpIn, null, aecTmpOut,
//                (short) (cacheSize ), (short) 40);
